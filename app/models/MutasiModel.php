...
class MutasiModel{
	...
	private $tbl_cust ="[bmnpt].[dbo].customer";
	private $db;
 ...
 public function DeleteDatatemptAll($data){
    ...
    public function ViewSudahposting($data){
        ...
        $query ="SELECT 
            ...
            a.CustAddress,
            (
                SELECT TOP 1 b.CustCoName
                FROM [bmnpt].[dbo].customer AS b
                WHERE b.CustomerID = a.CustomerID
            ) AS CustCoName
        FROM 
            mutasi2 AS a
        WHERE 
            a.SoTransacID='".$SoTransacID."' ;
        ...
        while(odbc_fetch_row($result2)){
            ...
            $datas[] =[
				...
				"CustName"      =>rtrim(odbc_result($result2,'CustName')),
				"CustAddress"   =>rtrim(odbc_result($result2,'CustAddress')),
				"CustCoName"    =>rtrim(odbc_result($result2,'CustCoName')),
                "detail"        =>$this->get_viewsDetelmutasi($SoTransacID),

                //rumus 2025 by wardi

                "Total"         =>number_format($this->total,0),
                "Tax"           =>number_format(($this->total  - ($this->total / $this->ppn)),0),
                "Sub_Total"     =>number_format(($this->total / $this->ppn),0)
    ...
    private function get_viewsDetelmutasi($SoTransacID){
        ...
        $query="SELECT PartId,PartName,transtype,keterangan,warehouse,warehouse2,Quantity,price,discount,amount 
        ...
        while(odbc_fetch_row($result2)){
           $this->total +=round(rtrim(odbc_result($result2,'amount')),0);
            $datas[]=[
                "PartId" => rtrim(odbc_result($result2,'PartId')),
                "PartName" => rtrim(odbc_result($result2,'PartName')),
                "Quantity" =>number_format(rtrim(odbc_result($result2,'Quantity')),0,",",","),
                "transtype" => rtrim(odbc_result($result2,'transtype')),
                "keterangan" => rtrim(odbc_result($result2,'keterangan')),
                "warehouse" => rtrim(odbc_result($result2,'warehouse')),
                "warehouse2" => rtrim(odbc_result($result2,'warehouse2')),
                "price" =>number_format(rtrim(odbc_result($result2,'price')),0,".","."),
                "discount" =>number_format(rtrim(odbc_result($result2,'discount')),2,".","."),
