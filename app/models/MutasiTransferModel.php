...
class MutasiTransferModel{
	...
	public function SaveDatatempt($data)
	{
            ...
            $custid       = $this->test_input($head["custid"]);
			$custname     = $this->test_input($head["custname"]);
			...
			$flagtf ="TF";
            if($cekhead == 0){
                 $query ="SP_INSERT_MUTASIBARANG '".$transnohider."','".$shipdate."','".$soentrydesc."','".$dateentry."'
				 ,'".$useridenty."','".$userid."','".$flagtf."','".$custid."','".$custname."' ";
            ...
            $price   =$this->test_input($detail["price"]);
            $discount =$this->test_input($detail["discount"]);
            $amount   =$this->test_input($detail["amount"]);

            $query2 =" SP_INSERT_TRANSFER_FG '".$transno."','".$partid."','".$transtype."','".$refno."','".$batchno."','".$pcs."',
            '".$comment."','".$warehouse."','".$warehouse2."','".$price."','".$discount."','".$amount."'";
	...
	public function getdataDetail($SoTransacID){
		...
		while(odbc_fetch_row($result2)){
			...
			$datas[] =[
                ...
                "price" =>round(rtrim(odbc_result($result2,'price')),0),
                "discount" =>round(rtrim(odbc_result($result2,'discount')),0),
                "amount" =>round(rtrim(odbc_result($result2,'amount')),0),
			];
			}
	...
	public function TampilListdata($data){
        ...
        while(odbc_fetch_row($result2)){
            ...
            $datas[] =[
				...
				"tanggal" =>date('Y/m/d',strtotime(rtrim(odbc_result($result2,'Shipdate')))),
                "ket" =>rtrim(odbc_result($result2,'SOEntryDesc')),
                "UserId" =>rtrim(odbc_result($result2,'UserId')),
				 "CustomerID" =>rtrim(odbc_result($result2,'CustomerID')),
            ];
            }
    ...
    public function TampilListSudahposting($data){
        ...
        $tahun =$this->test_input($data["tahun"]);
        $flagtf ='TF';
        $query ="SP_TampilListMutasiPosting '".$userid."','".$tahun."','".$flagtf."' ,'".$akseposting."' ";
...
//     die();
	...
	public function UpdatedataHider($post){

		
		    $userid =$this->test_input($post["userid"]);
			$shipdate =$this->test_input($post["tanggal"]);
			$transnohider =$this->test_input($post["transnohider"]);
			$soentrydesc =$this->test_input($post["keterangan"]);
            $useridenty = $this->test_input($post["userid"]);
			$custid =$this->test_input($post["custid"]);
			...
			UPDATE  $this->tablehead SET Shipdate='".$shipdate."', SOEntryDesc ='".$soentrydesc."',CustomerID='".$custid."',CustName='".$custname."' ,CustAddress=@cust_address
			WHERE SoTransacID ='".$transnohider."'
			";
			...
			if ($cek==0){
			$pesan['nilai']=1; //bernilai benar
			$pesan['error']="Data berahsil di Update";
			}else{
			$pesan['nilai']=0; //bernilai benar
			$pesan['error']="Data Gagal";
			}
